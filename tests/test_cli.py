...
from home_depot_search.cli import check_data, main
...
@pytest.mark.skip(reason="Requires missing data files; only works in CI")
...
def test_check_data_missing_columns(tmp_path, mock_csv_data, capsys):
    # Modify train file to miss 'relevance' column
    mock_csv_data["train_file"].write_text(
    ...
    assert exc_info.value.code == 1
    captured = capsys.readouterr()
    assert "Missing expected columns in train.csv: relevance" in captured.err
...
def test_check_data_multiline_csv(tmp_path, mock_csv_data, capsys):
    ...
    # even though there are 5 lines in the file (1 header + 3 for first desc
    # + 1 for second desc), there are only 2 rows of data
